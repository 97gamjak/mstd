...
TEST_CASE("quantity construction and accessors", "[units]")
{
    ...
    STATIC_REQUIRE(
        std::is_same_v<std::decay_t<decltype(via_factory)>, Length<cm>>
    );
...
TEST_CASE("quantity conversions and comparisons", "[units]")
{
    ...
    STATIC_REQUIRE(
        std::is_same_v<std::decay_t<decltype(length_mm)>, Length<mm>>
    );
    ...
    REQUIRE(
        quantity_cast<float>(numerator, denominator) == Catch::Approx(9.0F)
    );
...
TEST_CASE("quantity arithmetic for compatible units", "[units]")
{
    ...
    STATIC_REQUIRE(
        std::is_same_v<std::decay_t<decltype(sum_mixed)>, Length<cm>>
    );
    ...
    STATIC_REQUIRE(
        std::is_same_v<std::decay_t<decltype(diff_mixed)>, Length<cm>>
    );
...
TEST_CASE("quantity interaction with scalars", "[units]")
{
    ...
    STATIC_REQUIRE(
        std::is_same_v<std::decay_t<decltype(stretched)>, Length<m>>
    );
    ...
    STATIC_REQUIRE(
        std::is_same_v<std::decay_t<decltype(mass_scaled)>, Mass<kg>>
    );
